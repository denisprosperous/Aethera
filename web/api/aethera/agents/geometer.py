...
class IntrinsicGeometer:
    ...
    def solve_2d(self, graph):
        ...
        coords, stress, conv = smacof(delta, weight, dim=2, max_iter=self.max_iter, tol=self.tol)
        ...
        mf = IntrinsicManifold("2D", cm, stress,
            {nodes[i]: float(curv[i]) for i in range(n)}, "Agent 2")
        mf.convergence_residual = float(conv)
        return mf
    def solve_3d(self, graph):
        nodes = graph.nodes; n = len(nodes)
        if n < 4: raise ValueError(f"need >= 4 nodes, got {n}")
        delta, weight = self._build(graph, nodes)
        coords, stress, conv = smacof(delta, weight, dim=3, max_iter=self.max_iter, tol=self.tol)
        cm = {nodes[i]: Point3(coords[i,0], coords[i,1], coords[i,2]) for i in range(n)}
        mf = IntrinsicManifold("3D", cm, stress, {}, "Agent 2 (3D)")
        mf.convergence_residual = float(conv)
        return mf
    @staticmethod
