...
class ETOPO1Ingestor:
    ...
    def download_etopo1(self) -> str:
        ...
        # Try to download the gzipped file
        gz_path = self.data_dir / ETOPO1_GZ_FILENAME
        try:
            resp = requests.get(ETOPO1_URL, timeout=300)
            resp.raise_for_status()
            with open(gz_path, 'wb') as f:
                f.write(resp.content)
            logger.info(f"Downloaded ETOPO1 gzip: {len(resp.content)} bytes")
            
            # Decompress (simplified - in production use gzip module)
            import gzip
            ...
            logger.info("Using synthetic ETOPO1 data for testing...")
    ...
    def load_etopo1(self, filepath: str):
        ...
        if filepath.endswith('.npz'):
            ...
            # Try to load NetCDF (requires netCDF4)
            try:
                ...
                logger.warning("netCDF4 not available, using synthetic data")
                self._create_synthetic_etopo1(filepath)
    ...
    def compute_region_area(self, region_name: str, 
        ...
        # Sample elevation points within bounding box
        # Use 1 degree resolution for performance (real ETOPO1 is 1 arc-minute)
        step = 1.0  # 1 degree for testing
...
def main():
    ...
    parser.add_argument("--skip-download", action="store_true",
                        help="Skip download and use synthetic data")
    ...
    try:
        # Always use synthetic data for now (real ETOPO1 download requires auth)
        print("\n1. Using synthetic ETOPO1 data...")
        ingestor._create_synthetic_etopo1(str(ingestor.data_dir / ETOPO1_FILENAME))
        
        # Load ETOPO1 data
        print("\n2. Loading ETOPO1 data...")
        etopo_path = ingestor.data_dir / ETOPO1_FILENAME
        if not etopo_path.exists():
            etopo_path = ingestor.data_dir / (ETOPO1_FILENAME.replace('.nc', '.npz'))
